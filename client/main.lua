...
function checkOwner(vehicle)
	...
	FXCore.Functions.TriggerCallback('carremote:checkOwnedVehicle', function(result)
		if result then
			return true
		else
			return false
...
function unlockVehicle(vehicle)
	...
	if(IsPedInAnyVehicle(ply, true))then
		...
		FXCore.Functions.Notify(_U('unlocked'))
...
function lockVehicle(vehicle)
	...
	if(IsPedInAnyVehicle(ply, true))then
		...
		FXCore.Functions.Notify(_U('locked'))
...
function engineOn(vehicle)
	local ply = GetPlayerPed(-1)
	if(IsPedInAnyVehicle(ply, true))then
		FXCore.Functions.Notify(_U('engine_on'))
...
function engineOff(vehicle)
	local ply = GetPlayerPed(-1)
	if(IsPedInAnyVehicle(ply, true))then
		FXCore.Functions.Notify(_U('engine_off'))
	...
	if lastVehicle then
		...
		if Config.MaxRemoteRange >= vehicleDistance then
			...
			if lockStatus < 2 then
				...
				FXCore.Functions.Notify(_U('already_locked'))
			end
		else
			FXCore.Functions.Notify(_U('out_of_range'))
		end
	else
		FXCore.Functions.Notify(_U('not_connected'))
	end
	...
	if lastVehicle then
		...
		if Config.MaxRemoteRange >= vehicleDistance then
			...
			if lockStatus >= 2 then
				...
				FXCore.Functions.Notify(_U('already_unlocked'))
			end
		else
			FXCore.Functions.Notify(_U('out_of_range'))
		end
				...
				if vehicleDistance < 9 then
					FXCore.Functions.TriggerCallback('carremote:checkOwnedVehicle', function(result)
						...
						if result then
							...
							FXCore.Functions.Notify(_U('now_connected', plate))
							local vehicleDistance = GetDistanceBetweenCoords(GetEntityCoords(ply), GetEntityCoords(vehicle, false), true)
